...
/**
 * Created with IntelliJ IDEA. User: Sharon Date: 11/24/12 Time: 12:45 AM To
...
public class ActionBar
{
	...
	public static SlotState getSlotStateAt(final int index)
	{
		...
		return Settings.get(ID_SETTINGS_ABILITY_BASE + index) > -1
				? SlotState.ABILITY
				: SlotState.EMPTY;
	}
	...
	public static boolean isAbilityAvailable(final int index)
	{
		...
		if(slot != null)
		{
			...
			if(available != null && available.validate() && cooldown != null && cooldown
					.validate())
				return cooldown.getTextureId() == 14521 && available
						.getTextColor() == 16777215;
		}
	...
	public enum Slot
	{
		...
		{
			return Widgets.get(ID_WIDGET_ACTION_BAR, this.widgetChildAvailable);
		}
		
		public WidgetChild getCooldownWidget()
		{
			return Widgets.get(ID_WIDGET_ACTION_BAR, this.widgetChildCoolDown);
		}
		...
		public boolean isAvailable()
		{
			...
			return available != null && available.validate() && available
					.getTextColor() == 16777215;
		}
		
		public SlotState getSlotState()
		{
			final int item = Settings.get(ID_SETTINGS_ITEM_BASE + this.index);
			if(item > -1)
				return SlotState.ITEM;
			return Settings.get(ID_SETTINGS_ABILITY_BASE + this.index) > -1
					? SlotState.ABILITY
					: SlotState.EMPTY;
		}
		...
		public boolean activate(boolean sendKey)
		{
			...
			if(sendKey)
			{
				Keyboard.sendKey(Widgets
						.get(ID_WIDGET_ACTION_BAR, this.widgetChildText)
						.getText().toLowerCase().charAt(0));
				return true;
			}
			else
	...
	public enum Constitution_Abilities implements Ability
	{
		...
		SINGLE_WAY_WILDERNESS(132, "Single-way Wilderness", 10,
				AbilityType.BASIC);
	...
	public enum GeneralTab
	{
		...
		public boolean open()
		{
			...
			{
				final WidgetChild widgetChild = Widgets.get(
						ID_WIDGET_ABILITY_BOOK, this.widgetChildId);
				...
				if(isOpen())
				{
					final WidgetChild innerTab = Widgets.get(
							ID_WIDGET_ABILITY_BOOK, 42);
					if(innerTab.visible())
					{
						innerTab.click(true);
					}
	...
	public enum InnerTab
	{
		...
		InnerTab(final GeneralTab generalTab, final int regularMask,
				final int magicMask, final int widgetChildId)
		{
		...
		public boolean isOpen()
		{
			if(!this.generalTab.isOpen())
				return false;
			if(this.regularMask > 0) // Other tabs
				return (Settings.get(ID_SETTING_OPEN_TAB) & MASK_TAB) == this.regularMask;
			else
				// Magic tab
				return (Settings.get(ID_SETTING_OPEN_TAB) & MAGIC_MASK) == this.magicMask;
		}
